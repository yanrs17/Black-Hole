...
if (level == 1){
  ...
  var score = localStorage.level1score;//TODO
}
var gameSpeed = 30;
...
function move(i) {
    ...
    //TODO: draw use another function
    // ctx.drawImage(shapes[i].obj, newX, newY, 50, 50);
    canvasDraw(i,newX,newY);
    shapes[i].x = newX;
    shapes[i].y = newY;
}

function canvasDraw(i,x,y) {
  var temp = shapes[i];
  if (i == 0) { // moon
    ctx.beginPath();
    ...
    ctx.closePath();
    ctx.fill();
    // if (x<0||x>970) temp.dx =- temp.dx;
    // if (y<50||y>620) temp.dy =- temp.dy;
    // x += temp.dx;
    // y += temp.dy;
  }
  ...
  if (i == 1) { // spaceship
    ...
    ctx.fill();
    // if (x<40||x>1000) temp.dx =- temp.dx;
    // if (y<57||y>630) temp.dy =- temp.dy;
    // x += temp.dx;
    // y += temp.dy;
  }
  ...
  if(i == 2){ //rocket
    ...
    ctx.fill();
    // if (x<0||x>980) temp.dx =- temp.dx;
    // if (y<90||y>630) temp.dy =- temp.dy;
    // x += temp.dx;
    // y += temp.dy;
  }
  ...
  if (i == 6){ // planet
    ...
    // if (x<5||x>970) temp.dx =- temp.dx;
    // if (y<42||y>610) temp.dy =- temp.dy;
    // x += temp.dx;
    // y += temp.dy;
  }
  ...
  if (i ==9) { //debris2
    ...
    ctx.fill();
    // if (x<5||x>980) temp.dx =- temp.dx;
    // if (y<42||y>600) temp.dy =- temp.dy;
    // x += temp.dx;
    // y += temp.dy;
  }
}
...
function drawRandomObject(i) {
    ...
    //TODO: draw use another function
    canvasDraw(i,x,y);
    shapes.push(new Shape(x, y, xChange, yChange));
}
...
window.setInterval(function(){
  ...
  if ((shapes.length == 0) && (control != 1) && (time <59)){
    alert("you lose");
    control = 1;
  }
  if (time == 0 && (control != 3)) {
